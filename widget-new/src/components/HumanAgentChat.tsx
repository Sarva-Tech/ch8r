...
import { humanMessages, wsStatus, sendError, agentInfo, config, isOpen, activeMode, unreadHuman, chatroomsHuman } from '../store/signals';
...
function AgentList({ agent, onSelect }: { agent: AgentInfo | null; onSelect: () => void }) {
  const hasUnread = chatroomsHuman.value.some(c => c.has_unread);
...
export function HumanAgentChat() {
    ...
    if (chatroomIdRef.current !== 'new_chat') {
      apiClient.loadHistory(appUuid, chatroomIdRef.current, senderIdentifier).then(result => {
        if (result.ok) {
          ...
          sessionStore.setChatroomId(appUuid, 'new_chat', 'human');
        }
      });
    }
  ...
  const handleSelectAgent = () => {
    unreadHuman.value = 0;
    // Clear unread on all human chatrooms optimistically
    chatroomsHuman.value = chatroomsHuman.value.map(c => ({ ...c, has_unread: false }));
  ...
  return (
    <div class="flex flex-col h-full">
      <div class="flex items-center gap-3 px-3 py-2 border-b border-gray-100 bg-gray-50">
        <button onClick={handleBack} aria-label="Back to agents" class="p-1 rounded hover:bg-gray-200 transition-colors text-gray-600">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
            <path fill-rule="evenodd" d="M17 10a.75.75 0 0 1-.75.75H5.612l4.158 3.96a.75.75 0 1 1-1.04 1.08l-5.5-5.25a.75.75 0 0 1 0-1.08l5.5-5.25a.75.75 0 1 1 1.04 1.08L5.612 9.25H16.25A.75.75 0 0 1 17 10Z" clip-rule="evenodd" />
          </svg>
        </button>
        <div class="relative flex-shrink-0">
          <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold" style={{ backgroundColor: 'var(--ch8r-accent)' }}>
      ...
      {wsStatus.value === 'error' && (
        <div class="bg-red-50 border-b border-red-200 px-4 py-2 text-sm text-red-800 flex items-center justify-between">
          ...
          }} class="ml-3 text-xs font-medium underline hover:no-underline">Reconnect</button>
        </div>
      )}
      {sendError.value && <div class="bg-red-50 px-4 py-2 text-sm text-red-700">{sendError.value}</div>}
