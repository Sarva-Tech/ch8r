...
function Avatar({ senderIdentifier, isOwn }: { senderIdentifier: string; isOwn: boolean }) {
  ...
  if (isOwn) {
    return (
      <div class="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 bg-secondary text-primary">
        <WidgetUserIcon />
      </div>
    );
  }
...
export function MessageList({ messages, isTyping }: MessageListProps) {
  ...
  return (
    <div class="flex-1 overflow-y-auto flex flex-col gap-3 px-4 py-3">
      {messages.map((msg) => (
        <div
          key={msg.uuid}
          class={`flex items-start gap-2 ${msg.isOwn ? 'flex-row-reverse' : 'flex-row'}`}
        >
          <Avatar senderIdentifier={msg.senderIdentifier} isOwn={msg.isOwn} />
          <div class={`flex flex-col gap-1 max-w-[75%] ${msg.isOwn ? 'items-end' : 'items-start'}`}>
            <div
              class={`px-3 py-2 rounded-lg text-sm break-words overflow-hidden ${
                msg.isOwn
                  ? 'bg-primary text-primary-foreground'
                  : msg.senderIdentifier.startsWith('agent_llm')
