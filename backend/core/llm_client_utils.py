...
from core.consts import AI_ROLE_AI_AGENT, AI_ROLE_HUMAN_AGENT, AI_ROLE_USER, AI_ROLE_UNKNOWN, AI_ROLE_SYSTEM

from openai.types.shared_params import ResponseFormatJSONSchema

AI_ROLE_MAP = {
    "agent_llm": AI_ROLE_AI_AGENT,
    "reg_": AI_ROLE_HUMAN_AGENT,
    "anon_": AI_ROLE_USER
}
...
def add_kb_to_convo(
    conversation,
    kb_data,
    role = AI_ROLE_AI_AGENT
):
    if kb_data and kb_data != "NO_CONTEXT":
        conversation.append({
            "role": role,
            "content": "Use the following context to answer the user’s question. Only use this information; do not add outside knowledge.\n\n{kb_data}"
        })
    return conversation
